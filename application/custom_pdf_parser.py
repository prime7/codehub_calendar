...
def parsing_results(filename):
    ...
    tabula.convert_into(os.path.join(app.config['UPLOAD_FOLDER'], filename), os.path.join(
        app.config['UPLOAD_FOLDER'], outputname), output_format="csv", pages='all')
    ...
    arrForDatesAndEvents = []
    dictEntryOfDateAndEvent = {}
    nextColFlag = False
    # refining input
            ...
            for colToCheckEvent in row:
                ...
                if (event is not None):
                    ...
                    for colToCheckDate in row:
                        ...
                        # TO DO
                        if (date is not None):
                            dictEntryOfDateAndEvent[event.group(
                            )] = date.group()
                            arrForDatesAndEvents.append(
                                dictEntryOfDateAndEvent)
                            dictEntryOfDateAndEvent = {}
                            nextColFlag = True
                            break
    ...
    resultDict = {
        ...
        "EventsAndDates": arrForDatesAndEvents
    }
